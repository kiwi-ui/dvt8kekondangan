...
return (
    <div className={ `musicplayer text-center text-white me-3 mt-3 music position-absolute top-0  ` } onClick={ toggleSong } style={{zIndex: 100}}>
        <button className={ `d-flex align-items-center justify-items-center my-3 mx-1 position-relative bg-transparent rounded-circle border-0` }>
            <BsDiscFill className="bg-transparent rounded-circle fs-2 secondaryText" style={{ transform: isPlaying ? `rotate(${isRotating}deg)` : `rotate(${isRotating})` }}/>
        </button>
    </div>
    )
}
